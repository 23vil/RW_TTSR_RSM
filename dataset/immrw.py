...
class ToTensorBox(object): #Only used in debug mode
    def __call__(self, sample):
        LR, LR_sr, HR, Ref, Ref_sr, Box, srcFile = sample['LR'], sample['LR_sr'], sample['HR'], sample['Ref'], sample['Ref_sr'], sample['Box'], sample['srcFile']
        LR = LR.transpose((2,0,1)) #
        LR_sr = LR_sr.transpose((2,0,1))        
        HR = HR.transpose((2,0,1))
        Ref = Ref.transpose((2,0,1))
...
class TrainSet(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        Ref_sub = np.array([Ref_sub, Ref_sub, Ref_sub]).transpose(1,2,0) #make RGB image from greyscale imput----- Solve differently later!
...
class TestSet(Dataset):
    def __init__(self, args, ref_level='1', transform=transforms.Compose([ToTensor()])):
        ...
        self.ref_list = sorted(glob.glob(os.path.join(args.dataset_dir, 'test/ref/','*.tif'))) #path had to be altered manually
            #'*_' + ref_level + '*.tif')))   
        self.args = args
        if self.args.debug:
    ...
    def __getitem__(self, idx):
        ...
        #Ref = croper(Ref) #Take a Random 160 x 160 Crop
        Ref = np.array([Ref, Ref, Ref]).transpose(1,2,0) #make RGB image from greyscale imput----- Solve differently later!
        #Ref_sub = imread(self.ref_list[idx])
